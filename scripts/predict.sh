...
if [ $# != 1 ]; then
    echo "Must pass in test file, one tweet per line, first word being the label (any label if unknown))"
    exit 1
fi
...
python3 scripts/predict_logit.py $data.csv $data.predictions.txt $RESULTSDIR/model.pkl
